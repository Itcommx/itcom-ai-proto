...
def search_gdelt(payload: GDELTSearchRequest, client: GDELTClient | None = None) -> dict[str, object]:
    ...
    effective_max_results = max(1, min(effective_max_results, settings.external_max_results, 10))
    effective_language = (payload.language or settings.external_default_lang).strip().lower()
    effective_timespan = payload.timespan_hours or GDELTClient.default_timespan_hours(query)
    gdelt_client = client or GDELTClient()
    logger.info(
        "gdelt_search_params query=%s max_results=%s language=%s timespan_hours=%s start=%s end=%s",
        query,
    ...
    try:
        results = gdelt_client.search(
            ...
            timespan_hours=effective_timespan,
